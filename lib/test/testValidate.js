...
const validateRulesPersonPhones = [
...
describe('Validation rules over described Maps', function () {
    ...
    let errorResults = [];
    errorResults.push(new validate().run(validationRulesPerson, getObject()));
    getObject().person.phones.forEach(function(elem) {
        errorResults.push(new validate().run(validateRulesPersonPhones, elem));
    });
